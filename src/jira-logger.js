...
module.exports = robot => {
    ...
    function logHours(config) {
        ...
        const url = `${process.env['JIRA_API_URL']}/issue/${config.jiraNumber}/worklog`;
        ...
        return new Promise((resolve, reject) => {
            ...
            .post(JSON.stringify(worklog))((err, res, body) => {
    ...
    robot.respond(/log (.+) on ([^\s]+)(.*)?/i, response => {
        ...
        let encryptedUserpass = robot.brain.get(hubotUserID);
        ...
        let config = {
            ...
            projectToken: process.env['JIRA_PROJECT_TOKEN'],
    ...
    robot.respond(/add-user (.+) password (.[^\s]+)/, response => {
        ...
        try {
            let userpassBase64 = new Buffer(username + ':' + password).toString('base64');
            let encryptedUserpass = encryptText(userpassBase64, hubotUserID);
